...
1. **Tiered SDK block** (`src/disclosure.ts`): the assemble listener splices the rendered type block down to the core tool set (`DISCLOSURE_CORE_TOOLS` — bash, read, grep, glob, edit, write, ask_user_question, job_*, exit_plan_mode). Removed tools stay registered and callable; DSH's fixed `describe`/`call` declarations, intro, and fences remain available.
2. **Runtime discovery** (DSH `ToolRuntime`): every `run_code` execution receives `tools.describe(name)` from the executing agent's catalog snapshot and `tools.call({ name, args })` through the ordinary nested scheduler. The QuickJS provider only typechecks and executes the bindings DSH supplies, so concurrent sessions cannot race through a process-global latest catalog. The ported pi `typeErrorRecoveryHint` still names edit/write payload syntax failures.
3. **Capability advisory with combustion** (`src/advisory.ts`): inverse document frequency scores tool names plus first-sentence descriptions. The two-word gate, threshold, same-entry phrase window 2τ, strong band θ + 1, cooling warmth, scatter cap q = 1, score-before-budget selection, and burned-name limit (default 3) bound fires. Core orchestration names are ambient and never spend advisory fuel; optional matches become bounded `fabric-advisory` user messages with `tools.describe()` pointers.
