...
import { stateBinding } from './types.ts'
import {
...
export const DEFAULT_SCHEMA_CONFIG: FabricSchemaConfig = {
  mode: 'off',
...
export class SchemaController {
  ...
  async authorize(ref: string, parentToolCallId: string): Promise<void> {
    ...
    if (this.config.mode === 'audit') {
      await this.#publish('would_block', { ref, parentToolCallId, message }).catch(() => undefined)
      return
    }
    await this.#publish('blocked', { ref, parentToolCallId, message }).catch(() => undefined)
    throw new Error(message)
  }

  status(parentToolCallId?: string): Record<string, unknown> {
    const workspace = this.#workspaceEntry()
    const hypotheses = this.#listStates(HYPOTHESIS_PREFIX)
      .map((entry) => entry.value as unknown as SchemaHypothesisRecord)
    ...
    return {
      ...
      generation: (workspace?.value as unknown as SchemaWorkspaceRecord | undefined)?.generation ?? 0,
      lastOutcome: (workspace?.value as unknown as SchemaWorkspaceRecord | undefined)?.lastOutcome ?? null,
      hypotheses,
    }
  }
  ...
  async hypothesize(
    ...
    const snapshot = snapshotWorkspace(this.cwd, this.#exclusions)
    ...
    await this.#cas(`${HYPOTHESIS_PREFIX}${record.id}`, 0, record)
    this.#activeHypotheses.set(record.id, parentToolCallId)
    return {
  ...
  async verify(
    ...
    const record = entry.value as unknown as SchemaHypothesisRecord
    this.#assertInvocation(record.parentToolCallId, parentToolCallId)
    ...
    try {
      before = snapshotWorkspace(this.cwd, this.#exclusions)
    } catch (error) {
    ...
    const results = await this.#verifyEvidence(record.evidence)
    let after: WorkspaceSnapshot
    try {
      after = snapshotWorkspace(this.cwd, this.#exclusions)
    } catch (error) {
    ...
    await this.#cas(certificateKey, 0, certificateRecord)
    try {
      await this.#cas(entry.key, entry.version, { ...record, status: 'verified', updatedAt: Date.now() })
    } catch (error) {
      const certificateEntry = this.#getState(certificateKey)
      if (certificateEntry) {
        await this.#cas(certificateKey, certificateEntry.version, { ...certificateRecord, status: 'aborted' })
      }
      throw error
    }
    this.#activeCertificates.set(certificateRecord.tokenHash, parentToolCallId)
    try {
  ...
  async commit(
    ...
    try {
      ...
      const certificateEntry = this.#getState(`${CERTIFICATE_PREFIX}${tokenHash}`)
      if (!certificateEntry) throw new Error('Unknown Schema certificate')
      const certificate = certificateEntry.value as unknown as SchemaCertificateRecord
      ...
      this.#assertInvocation(certificate.parentToolCallId, parentToolCallId)
      ...
      const hypothesis = hypothesisEntry.value as unknown as SchemaHypothesisRecord
      ...
      const baseline = snapshotWorkspace(this.cwd, this.#exclusions)
      ...
      await this.#cas(certificateEntry.key, certificateEntry.version, { ...certificate, status: 'consumed', consumedAt: Date.now() })
      consumed = true
      journal.status = 'applying'
      atomicJsonWrite(journalPath, journal)
      const afterConsume = snapshotWorkspace(this.cwd, this.#exclusions)
      ...
      const applied = snapshotWorkspace(this.cwd, this.#exclusions)
      this.#assertNoOutsideDrift(baseline, applied, new Set(declared.keys()))
      const postconditionResults = await this.#verifyEvidence(input.postconditions)
      const afterPostconditions = snapshotWorkspace(this.cwd, this.#exclusions)
      ...
      await this.#cas(WORKSPACE_KEY, workspaceEntry?.version ?? 0, {
      ...
      try {
        await this.#cas(hypothesisEntry.key, hypothesisEntry.version, { ...hypothesis, status: 'committed', updatedAt: Date.now() })
      } catch {
  ...
  async abort(
    ...
    const hypothesisEntry = this.#requireHypothesis(input.hypothesisId)
    const hypothesis = hypothesisEntry.value as unknown as SchemaHypothesisRecord
    this.#assertInvocation(hypothesis.parentToolCallId, parentToolCallId)
    ...
    if (input.certificate) {
      ...
      this.#assertInvocation(certificate.parentToolCallId, parentToolCallId)
      if (certificate.status !== 'active') throw new Error(`Schema certificate is ${certificate.status}`)
      await this.#cas(certificateEntry.key, certificateEntry.version, { ...certificate, status: 'aborted' })
      this.#activeCertificates.delete(tokenHash)
    }
    await this.#cas(hypothesisEntry.key, hypothesisEntry.version, { ...hypothesis, status: 'aborted', updatedAt: Date.now() })
    this.#activeHypotheses.delete(hypothesis.id)
    await this.#publish('aborted', { hypothesisId: hypothesis.id, parentToolCallId })
  ...
  async endInvocation(parentToolCallId: string): Promise<void> {
    for (const [tokenHash, invocation] of [...this.#activeCertificates]) {
      ...
      const entry = this.#getState(`${CERTIFICATE_PREFIX}${tokenHash}`)
      if (entry) {
        const record = entry.value as unknown as SchemaCertificateRecord
        if (record.status === 'active') {
          try {
            await this.#cas(entry.key, entry.version, { ...record, status: 'abandoned' })
          } catch {
    ...
    for (const [hypothesisId, invocation] of [...this.#activeHypotheses]) {
      if (invocation !== parentToolCallId) continue
      const entry = this.#getState(`${HYPOTHESIS_PREFIX}${hypothesisId}`)
      if (entry) {
        const record = entry.value as unknown as SchemaHypothesisRecord
        if (record.status === 'active' || record.status === 'verified') {
          try {
            await this.#cas(entry.key, entry.version, { ...record, status: 'abandoned', updatedAt: Date.now() })
          } catch {
      ...
      try {
        if (item.kind === 'trusted_command') {
          ...
          if (!command) {
            ...
            results.push(await this.#runTrustedCommand(item, command))
          }
          continue
        }
  ...
  #runTrustedCommand(
    ...
    return new Promise((resolve) => {
      ...
      child.once('error', (error) => finish('error', error.message, null))
  ...
  async #recordFailedOutcome(
    ...
    try {
      ...
      await this.#cas(WORKSPACE_KEY, entry?.version ?? 0, {
        generation: (entry?.value as unknown as SchemaWorkspaceRecord | undefined)?.generation ?? 0,
        lastOutcome: outcome,
        lastTransactionId: transactionId,
        updatedAt: Date.now(),
      } satisfies SchemaWorkspaceRecord)
  ...
  #workspaceEntry(): FabricStateRecord | undefined {
    return this.#getState(WORKSPACE_KEY)
  }

  #requireHypothesis(id: string): FabricStateRecord {
    const entry = this.#getState(`${HYPOTHESIS_PREFIX}${id}`)
    if (!entry) throw new Error(`Unknown Schema hypothesis: ${id}`)
    return entry
  }

  #assertInvocation(expected: string, actual: string): void {
    if (expected !== actual) throw new Error('Schema artifact belongs to a different tool invocation')
  }
  ...
  #publish(kind: string, data: unknown): Promise<SchemaMeshEvent> {
    return this.#publishTopic(SCHEMA_TOPIC, kind, data)
  }

  async #publishTopic(topic: string, kind: string, data: unknown): Promise<SchemaMeshEvent> {
    await this.#ensureTopic(topic)
    const events = this.#readTopic(topic)
    const sequence = events.reduce((highest, event) => Math.max(highest, event.sequence), 0) + 1
    const message = await this.mesh.publish(topicId(topic), {
      kind,
      sequence,
      ts: Date.now(),
      from: this.identity,
  ...
  #readTopic(topic: string): SchemaMeshEvent[] {
    try {
      return this.mesh
        .topicMessages(topicId(topic), 500)
  ...
  #getState(key: string): FabricStateRecord | undefined {
    return this.mesh.getState(FabricStateKey(key))
  }

  async #cas(key: string, expectedVersion: number, value: unknown): Promise<FabricStateRecord> {
    return this.mesh.compareAndSwap(FabricStateKey(key), expectedVersion, value as FabricJsonValue)
  }

  #listStates(prefix: string): FabricStateRecord[] {
    return this.mesh
      .snapshot()
      .states
