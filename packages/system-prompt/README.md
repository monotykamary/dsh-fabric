...
- Tool schemas, dynamic runtime context, and variables are never filtered. DSH ToolRuntime owns run-scoped `tools.describe`/`tools.call`; the prompt plugin only splices optional declarations and scores advisory hints with inverse document frequency, a strong band, cooling warmth, and score-ranked fire selection.
