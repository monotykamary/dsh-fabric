...
/**
 * The disclosure catalog service: the latest agent-scoped tool schemas,
 * published by the advisory's pre-step listener so `tools.describe()` inside
 * Code Mode can resolve contracts for tools the prompt no longer lists.
 *
 * A single-process deployment mounts one standing agent scope per preset, so
 * the latest catalog covers every session of that preset. Multi-preset
 * processes keep per-agent catalogs keyed by agent id, and `describe()`
 * falls back to the most recently published one (a deliberate v1
 * approximation: the runtime has no agent handle at binding time).
 */
export class DisclosureStore {
  private readonly byAgent = new Map<string, Map<string, DisclosureEntry>>()
  private latest = new Map<string, DisclosureEntry>()

  /** Publish the catalog visible to one agent (replaces that agent's view). */
  update(agentId: string, entries: readonly DisclosureEntry[]): void {
    const view = new Map(entries.map(entry => [entry.name, entry]))
    this.byAgent.set(agentId, view)
    this.latest = view
  }

  /** The agent's own catalog, when known. */
  catalogFor(agentId: string): readonly DisclosureEntry[] {
    return [...(this.byAgent.get(agentId) ?? new Map<string, DisclosureEntry>()).values()]
  }

  /** The most recently published catalog (the describe() fallback). */
  catalog(): readonly DisclosureEntry[] {
    return [...this.latest.values()]
  }

  /** Resolve one tool contract from the latest catalog. */
  describe(name: string): DisclosureEntry | undefined {
